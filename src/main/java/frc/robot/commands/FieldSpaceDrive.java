...
public class FieldSpaceDrive extends CommandBase {
    ...
    public void execute() {
        ...
        double xval = Math.max(Math.min(joystickHandler.getAxis1() * -speedRate * 5, 5), -5);
        double yval = Math.max(Math.min(joystickHandler.getAxis0() * -speedRate * 5, 5), -5);
        double spinval = Math.max(Math.min(joystickHandler.getAxis5() * -turnRate * 5, 5), -5);
